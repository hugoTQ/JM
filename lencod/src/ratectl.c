...
void rc_init_seq()
{
   ...
   /*basic unit layer rate control*/
	...
	// 以下为 JVT-H017 公式 4 的内容，L1/L2/L3 取值与提案中不同
	...
	if((fieldpic||topfield)&&targetcomputation)
	{
		...
		case P_SLICE:
			...
			if(img->NumberofCodedPFrame==1)
				AWp=Wp;		// 第二个 P 图像平均复杂度加权值直接等于第一个 P 图像复杂度
			// JVT-H017 公式 12 中的第一个式子，AWp 为 P 图像平均复杂度加权值，即为等式左边项
			...
			// 有非参考图像则使用 JVT-H017 公式 11 计算虚拟缓冲满度目标值
			if(input->successive_Bframe>0)
			{
		...
		case B_SLICE:
			...
			else if(img->NumberofCodedBFrame>1)
			{
				...
				// JVT-H017 公式 12 中的第二个式子，AWb 为 B 图像平均复杂度加权值，即为等式左边项
...
{
  // 计算当前 GOP 剩余比特数，JVT-H017 公式 2
  R-= nbits; /* remaining # of bits in GOP */
  // 计算虚拟缓冲满度实际值，JVT-H017 公式 3
  ...
  // 计算虚拟缓冲上下界，JVT-H017 公式 18、19(other)
...
void rc_update_pict_frame(int nbits)
{
	...
	else
	{
		if(img->type==P_SLICE)
		{
				...
				||(img->FieldControl==0))
			{
				...
				X=(int)floor(nbits*Avem_Qc+0.5);	// 计算 P 图像复杂度，JVT-H017 公式 12 中的第三个式子
			}
		}
	...
	case B_SLICE:
		Xb = X;
		Nb--;
		Wb=Xb/THETA;	// 计算 P 图像复杂度，JVT-H017 公式 12 中的第四个式子
		...
		else if(img->type==P_SLICE)
		{
			...
			else
			{
				...
				else
				{
					...
					TotalBasicUnitBits=img->NumberofBasicUnitHeaderBits+img->NumberofBasicUnitTextureBits;	// 当前图像中前一个编码基本单元所用比特数
					T -=TotalBasicUnitBits;	// 下一个待编码基本单元可用比特数
					img->NumberofBasicUnitHeaderBits=0;
					img->NumberofBasicUnitTextureBits=0;
					if(T<0)
					{
					...
					else
					{
						...
						m_Bits =(int)(T*CurrentFrameMAD*CurrentFrameMAD/TotalBUMAD);	// JVT-H017 公式 24
						/*compute the number of texture bits*/
						m_Bits -=PAveHeaderBits2;	// JVT-H017 公式 26
...
void updateRCModel ()
{
  ...
  if(img->type==P_SLICE)
  {
	  ...
	  else
	  {
		  ...
		  if(CodedBasicUnit>0)
		  {
			  // JVT-H017 公式 25，PAveHeaderBits1 即第一个式子的左边项，PAveHeaderBits2 即第二个式子的左边项
