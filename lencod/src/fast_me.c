...
static pel_t (*PelY_14) (pel_t**, int, int, int, int);  //返回像素
...
//计算一个块的代价，然后与以前计算的最小代价做比较，返回两者中较小者
//传入的mcost是当前块的MV cost，加入本函数中计算的SAD得到当前块的总代价
 ...
 ************************************************************************
 */
void   decide_intrabk_SAD()//chj 判断周围块的编码模式
{
  if (img->type != 0)
  {
